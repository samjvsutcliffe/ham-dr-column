...
for f in files:
    ...
    for solver,row in df_0.groupby("solver"):      
        df = row.sort_values("lstps")
        time_total = df.groupby("lstps")
        time = df.groupby("lstps")["time"].mean()
        lstps = time.keys()#df["lstps"].values
        plt.plot(lstps,time,label=solver)
        plt.scatter(df["lstps"].values,df["time"].values,label=solver)
plt.xscale("log")
plt.yscale("log")
plt.xlabel("Loadstep count")
